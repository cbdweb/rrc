/*
 Script: iphoneinterface.js
 
 Purpose:
 	Manage the interface of the iphone enabled Route Rise Calculator
 	Does not contain common functions, or Google map management code.
 	
 Author:
 	John Porra, September 2010, Copyright CBDWeb 2010
 */
...
var display = {
	...
	initialize : function(){
		...
		$('button3').getElement('a').addEvent('click', this.toggleSummary.bind(this));
	...
	sizeScreen : function(){
		scrollTo(0,0);
		// return;
		this.screenSize = $(document.body).getComputedSize().height;
		$('wrap').setStyle('height', this.screenSize );
		$('clipper').setStyle('height', this.screenSize  );
		$('content').setStyle('height', this.screenSize );
		$('map').setStyle('height', this.screenSize );
		$('settings').setStyle('height', this.screenSize );
		$('results').setStyle('height', this.screenSize );
	},
	fadeSplash : function(){
	...
	setActiveButton: function( id ){
		this.sizeScreen();  //  make sure we've alwats got the maximum screen available.
